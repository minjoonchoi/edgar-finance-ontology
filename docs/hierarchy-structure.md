...
- **출처 추적**: `fromFiling`으로 관측값을 원본 SEC 공시 문서와 연결 ⭐ NEW

### 4. 공시 문서 계층 구조 ⭐ NEW (v1.1.0)
...
│
├── hasQuarter: xsd:integer [선택적, 값: 1~4] ⭐ NEW (v1.1.1)
│   └── 의미: 회계 분기 (1, 2, 3, 4)
...
containsObservation ≡ inverse(fromFiling) ⭐ NEW (v1.1.0)
```

**의미**: 양방향 탐색 가능
- `company1 hasObservation obs1` ⇔ `obs1 ofCompany company1`
- `metric1 observedBy obs1` ⇔ `obs1 observesMetric metric1`
- `filing1 containsObservation obs1` ⇔ `obs1 fromFiling filing1` ⭐ NEW
...
#### 패턴 5: Filing 문서에서 데이터 추출 ⭐ NEW (v1.1.0)
...
#### 패턴 6: 관측 시점 vs 보고 시점 분석 ⭐ NEW (v1.1.0)
...
**최초 작성일**: 2025-01-19
**최종 수정일**: 2025-01-20
**기반 온톨로지 버전**: efin:Ontology 1.1.1-clarity-improvements

**v1.1.1 주요 변경사항** (2025-01-20):
- `hasQuarter` 속성 추가: 분기별 데이터 지원 (섹션 2.2)
- 분기별 데이터 조회 SPARQL 패턴 추가 (패턴 8)
- DebtCurrent와 ShortTermDebt equivalentClass 관계 명시
- InvestedCapital, DebtToEquity 공식의 명확성 개선

**v1.1 주요 변경사항**:
- Filing 클래스 계층 추가 (TenK, TenQ, EightK, TwentyF)
- Filing 관련 객체 속성 추가 (fromFiling, filedBy, containsObservation)
- Filing 관련 데이터 속성 추가 (accessionNumber, filingDate, fiscalPeriod, documentUrl, acceptanceDateTime)
- 시간 속성 명확화 (관측 시점 vs 보고 시점 구분)
- Filing 관련 SPARQL 쿼리 패턴 추가
