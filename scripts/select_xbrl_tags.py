...
def emit_efin_ttl(companies: List[dict], observations: List[dict], outfile: str, benchmarks: List[dict] = None, rankings: List[dict] = None):
    ...
    # Filing 인스턴스 추적 (중복 방지)
    filings_seen: Dict[str, dict] = {}  # accn -> {iri, cik, form, end, fiscal_period}
    ...
    for o in observations:
        ...
        else:  # period_type == "instant" (이미 검증됨)
        ...
        # Filing 인스턴스 생성 및 연결 (필수 함수형 속성)
        if accn:
            accn_clean = accn.replace("-", "")
            filing_iri = f"efin:Filing{_iri_safe(accn_clean)}"
            if accn not in filings_seen:
                # fiscalPeriod 추론
                fiscal_period = ""
                if form:
                    form_upper = form.upper()
                    if "10-K" in form_upper or "20-F" in form_upper:
                        fiscal_period = "FY"
                    elif "10-Q" in form_upper:
                        if quarter is not None:
                            fiscal_period = f"Q{quarter}"
                        else:
                            fiscal_period = "Q1"  # 기본값
            ...
            # Observation에 Filing 연결
            lines.append(f"  efin:fromFiling {filing_iri} ;")
        ...
        # composite_name은 스키마에 정의되지 않아 제거됨
        if reason:
            lines.append(f'  efin:hasSelectionReason "{_ttl_escape(reason)}" ;')
        if str(confidence) != "":
            try:
                c = float(confidence)
                # hasConfidence도 xsd:double 범위
                lines.append(f'  efin:hasConfidence "{c}"^^xsd:double ;')
            except Exception:
                pass
        if components:
    ...
    if filings_seen:
        lines.append("")
        lines.append("# Filing 인스턴스")
        for accn, filing_info in sorted(filings_seen.items()):
            filing_iri = filing_info["iri"]
            cik = filing_info["cik"]
            form = filing_info["form"]
            fiscal_period = filing_info["fiscal_period"]
            
            # Filing 클래스 타입 결정
            filing_class = "efin:Filing"
            if form:
                form_upper = form.upper()
                if "10-K" in form_upper or "10-K/A" in form_upper:
                    filing_class = "efin:TenK"
                elif "10-Q" in form_upper or "10-Q/A" in form_upper:
                    filing_class = "efin:TenQ"
                elif "20-F" in form_upper or "20-F/A" in form_upper:
                    filing_class = "efin:TwentyF"
                elif "8-K" in form_upper or "8-K/A" in form_upper:
                    filing_class = "efin:EightK"
            
            lines.append(f"{filing_iri} a {filing_class} ;")
            lines.append(f'  efin:accessionNumber "{_ttl_escape(accn)}" ;')
            lines.append(f"  efin:filedBy efin:CIK{cik} ;")
            if form:
                lines.append(f'  efin:hasFormType "{_ttl_escape(form)}" ;')
            if fiscal_period:
                lines.append(f'  efin:fiscalPeriod "{_ttl_escape(fiscal_period)}" ;')
            lines[-1] = lines[-1].rstrip(" ;")
            lines.append(".")
