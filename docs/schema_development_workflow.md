# EDGAR-FIN 2024 온톨로지 설계 및 개발 방식 (ODP 기반 통합 문서)
...
|---|---|---|---|
| **재사용** | `efin:LegalEntity` | 상속: `fibo-be:LegalEntity` | CQ “Which companies reported Revenue?”에서 [companies] → 기업 명사구 |
|  | `efin:RegulatoryFiling` | 상속: `fro:RegulatoryFiling` | CQ “In which filings was NetIncome reported?”의 [filings] 명사구 |
| **핵심 (자체)** | `efin:FinancialObservation` | 자체 정의 | CQ 구조 전체(“companies–reported–Revenue–FY2024”)의 삼항 패턴에서 Observation 패턴 도출 |
|  | `efin:CanonicalMetric` | 자체 정의 | CQ의 [Revenue], [Operating Income], [Assets] 등 명사구에서 추출 |
|  | `efin:XbrlConcept` | 자체 정의 | 자연어 “tag”, “concept”, “XBRL element”에서 명시적 등장 |
|  | `efin:TagMapping` | 자체 정의 | “map”, “corresponds to” 구문에서 매핑 개념 도출 |
| **시간·맥락 (자체)** | `efin:Period`, `efin:DurationPeriod`, `efin:InstantPeriod` | 자체 정의 | CQ “in FY2024” / “at 2024-12-31” 에서 기간 표현 추출 |
|  | `efin:Currency` | 자체 정의 | CQ “in USD”에서 화폐 표현 추출 |
|  | `efin:AccountingStandard` | 자체 정의 | “under US-GAAP / IFRS” 문장에서 회계 기준 명사구 추출 |
|  | `efin:Industry` | 자체 정의 | “for banks”, “for REITs” 등 업종 한정 문장에서 명사구 추출 |
| **파생 정의 클래스** | `efin:RevenueObservation` 등 | 정의 클래스 | CQ “Which observations are revenues?” → 정의식 생성 |
...
|---|---|---|
| `efin:forEntity` | FinancialObservation → LegalEntity | CQ “Which companies…”의 주어 [companies]와 Observation 주체 연결 |
| `efin:forMetric` | FinancialObservation → CanonicalMetric | CQ “reported [Revenue]”의 목적어 [Revenue] |
| `efin:hasPeriod` | FinancialObservation → Period | CQ “in [FY2024]” 시간구 표현 |
| `efin:hasCurrency` | FinancialObservation → Currency | “in [USD]” 화폐 명시 |
| `efin:usesAccountingStandard` | FinancialObservation → AccountingStandard | “under [US-GAAP/IFRS]” |
| `efin:basedOnConcept` | FinancialObservation → XbrlConcept | 자연어 “tagged as [us-gaap:Revenue]” 구문 |
| `efin:computedFrom` | FinancialObservation → FinancialObservation | “Operating Income derived from Revenue – Expenses” 문장 구조 |
| `efin:normalizedAs` | FinancialObservation → CanonicalMetric | “normalized to Revenue” 구문 |
| `efin:mapsFrom`, `efin:mapsTo` | TagMapping → (XbrlConcept / CanonicalMetric) | “maps from”, “corresponds to” 표현 |
| `efin:hasValue` | FinancialObservation → xsd:decimal | CQ “What was the value of Revenue?” 의 [value] 명사구 |
| `efin:periodStart`, `efin:periodEnd`, `efin:fiscalYear` | Period → xsd:date / xsd:gYear | “from Jan 1 to Dec 31”, “FY2024” 시간 표현 |
