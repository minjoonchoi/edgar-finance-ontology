...
def emit_efin_ttl(companies: List[dict], observations: List[dict], outfile: str, benchmarks: List[dict] = None, rankings: List[dict] = None):
    """
    ...
    for c in companies:
        ...
        comp_iri = f"efin:CIK{cik}" if cik else f"efin:Company-{_iri_safe(sym or name)}"
    ...
    for o in observations:
        ...
        obs_id_core = f"{cik}-{fy}-{metric}-{end or 'NA'}"
        obs_iri = f"efin:obs-{_iri_safe(obs_id_core)}"

        # periodType에 따라 적절한 클래스 타입 지정 (정의 클래스 활용)
        if period_type == "duration":
            observation_class = "efin:DurationObservation"
        elif period_type == "instant":
            observation_class = "efin:InstantObservation"
        else:
            # 기본값은 MetricObservation (하지만 스키마 제약 위반 가능)
            observation_class = "efin:MetricObservation"

        lines.append(f"{obs_iri} a {observation_class} ;")
        ...
        lines.append(f'  efin:hasFiscalYear "{_ttl_escape(fy)}"^^xsd:gYear ;')
        ...
        # periodEnd: 선택적
        if end:
            lines.append(f'  efin:hasPeriodEnd "{_ttl_escape(end)}"^^xsd:date ;')
        
        # unit: 선택적
        if unit:
            lines.append(f'  efin:hasUnit "{_ttl_escape(unit)}" ;')
        
        # 필수 속성: numericValue (검증 완료)
        try:
            v = float(value)
            lines.append(f"  efin:hasNumericValue {v} ;")
        except Exception:
            lines.append(f'  efin:hasNumericValue "{_ttl_escape(str(value))}" ;')
        ...
        # 나머지 optional 속성들
        if form:
            lines.append(f'  efin:hasFormType "{_ttl_escape(form)}" ;')
        if accn:
            lines.append(f'  efin:hasAccessionId "{_ttl_escape(accn)}" ;')
        if source_type:
            lines.append(f'  efin:hasSourceType "{_ttl_escape(source_type)}" ;')
        if selected_tag:
            lines.append(f'  efin:hasSelectedTag "{_ttl_escape(selected_tag)}" ;')
        if composite_name:
            lines.append(f'  efin:hasCompositeName "{_ttl_escape(composite_name)}" ;')
        if reason:
            lines.append(f'  efin:hasSelectionReason "{_ttl_escape(reason)}" ;')
        if str(confidence) != "":
            try:
                c = float(confidence)
                lines.append(f"  efin:hasConfidence {c} ;")
            except Exception:
                pass
        if components:
            lines.append(f'  efin:hasComponentsText "{_ttl_escape(components)}" ;')
    ...
    if benchmarks:
        ...
        for b in benchmarks:
            ...
            if industry:
                ...
                lines.append(f'  efin:forFiscalYear "{fy}"^^xsd:gYear ;')
            elif sector:
                # 섹터별 벤치마크
                bench_iri = f"efin:SectorBenchmark{_iri_camel_case(sector)}{_iri_camel_case(metric)}{fy}"
                lines.append(f"{bench_iri} a efin:SectorBenchmark ;")
                lines.append(f"  efin:forSector efin:Sector{_iri_camel_case(sector)} ;")
                lines.append(f"  efin:forMetric efin:{_iri_safe(metric)} ;")
                lines.append(f'  efin:forFiscalYear "{fy}"^^xsd:gYear ;')
            else:
                # 전체 벤치마크 (industry와 sector가 모두 빈 값)
                bench_iri = f"efin:SectorBenchmarkAll{_iri_camel_case(metric)}{fy}"
                lines.append(f"{bench_iri} a efin:SectorBenchmark ;")
            ...
            if avg is not None:
                lines.append(f"  efin:hasAverageValue {avg} ;")
            if median is not None:
                lines.append(f"  efin:hasMedianValue {median} ;")
            if max_val is not None:
                lines.append(f"  efin:hasMaxValue {max_val} ;")
            if min_val is not None:
                lines.append(f"  efin:hasMinValue {min_val} ;")
            if p25 is not None:
                lines.append(f"  efin:hasPercentile25 {p25} ;")
            if p75 is not None:
                lines.append(f"  efin:hasPercentile75 {p75} ;")
    ...
    if rankings:
        ...
        # 랭킹별로 그룹화: (scope_type, scope_value, metric, ranking_type) -> [(cik, rank), ...]
        # scope_type: "industry", "sector", "all"
        ranking_groups: Dict[Tuple[str, str, str, str], List[Tuple[str, int]]] = {}
        ...
        for r in rankings:
            ...
            key = (scope_type, scope_value, metric, ranking_type)
            if key not in ranking_groups:
                ranking_groups[key] = []
            
            ranking_groups[key].append((cik, rank_int))
        
        # 각 랭킹 그룹에 대해 인스턴스 생성
        for (scope_type, scope_value, metric, ranking_type), companies_list in ranking_groups.items():
            if not fy_ranking:
                continue
            
            # 인스턴스 IRI 생성
            if scope_type == "industry":
                ranking_iri = f"efin:TopRanking{_iri_camel_case(scope_value)}{_iri_camel_case(metric)}{ranking_type}{fy_ranking}"
            elif scope_type == "sector":
                ranking_iri = f"efin:TopRankingSector{_iri_camel_case(scope_value)}{_iri_camel_case(metric)}{ranking_type}{fy_ranking}"
            else:  # scope_type == "all"
                ranking_iri = f"efin:TopRankingAll{_iri_camel_case(metric)}{ranking_type}{fy_ranking}"
            
            lines.append(f"{ranking_iri} a efin:TopRanking ;")
            ...
            lines.append(f"  efin:forMetric efin:{_iri_safe(metric)} ;")
            lines.append(f'  efin:forFiscalYear "{fy_ranking}"^^xsd:gYear ;')
            lines.append(f'  efin:hasRankingType "{_ttl_escape(ranking_type)}" ;')
            lines.append(".")
            
            # 각 회사를 랭킹에 연결
            for cik, rank in companies_list:
...
def compute_benchmarks(tags_csv_path: str, fy: int) -> List[dict]:
    ...
    sector_groups: Dict[Tuple[str, str], List[float]] = {}  # (sector, metric) -> values
    ...
    for row in rows:
        ...
        # 섹터별 통계 (해당 섹터의 모든 산업 데이터 집계)
        if sector:
            key_sector = (sector, metric)
            if key_sector not in sector_groups:
                sector_groups[key_sector] = []
            sector_groups[key_sector].append(value)
    ...
    # 섹터별 벤치마크 계산
    for (sector, metric), values in sector_groups.items():
        if len(values) < 2:
            continue
...
def compute_rankings(tags_csv_path: str, benchmarks_csv_path: str, fy: int) -> List[dict]:
    ...
    """
    # 모든 메트릭 포함 (BASE_METRICS + DERIVED_METRICS)
    KEY_METRICS = BASE_METRICS + DERIVED_METRICS
    ...
    for (industry, metric), companies in industry_groups.items():
        ...
        # Top10, Top50, Top100 생성
        for top_n, ranking_type in [(10, "Top10"), (50, "Top50"), (100, "Top100")]:
    ...
    # 섹터별 랭킹 계산
    sector_groups: Dict[Tuple[str, str], List[Tuple[str, str, float]]] = {}  # (sector, metric) -> [(cik, symbol, value), ...]
    ...
    for (cik, industry, sector), metrics in company_metrics.items():
        ...
        for metric in KEY_METRICS:
            if metric not in metrics:
                continue
            value = metrics[metric]
            if sector:  # 섹터가 있는 경우만
                key = (sector, metric)
                if key not in sector_groups:
                    sector_groups[key] = []
                sector_groups[key].append((cik, symbol, value))
    
    # 각 섹터-메트릭 조합에 대해 랭킹 생성
    for (sector, metric), companies in sector_groups.items():
        # 값 기준 내림차순 정렬 (높은 값이 좋은 지표)
        # 단, DebtToEquity는 낮은 값이 좋으므로 오름차순
        reverse = metric != "DebtToEquity"
        sorted_companies = sorted(companies, key=lambda x: x[2], reverse=reverse)
    ...
    # 섹터별 종합 점수 기반 랭킹 계산
    sector_composite: Dict[str, List[Tuple[str, str, float]]] = {}  # sector -> [(cik, symbol, composite_score), ...]
    ...
    for (cik, industry, sector), metrics in company_metrics.items():
        ...
        for (cik2, ind, sec), metrics2 in company_metrics.items():
            ...
            for metric in KEY_METRICS:
                if metric not in metrics2:
                    continue
                if metric not in sector_values:
                    sector_values[metric] = []
                sector_values[metric].append(metrics2[metric])
        
        # 정규화된 점수 계산
        composite_score = 0.0
        for metric in KEY_METRICS:
            if metric not in metrics or metric not in sector_values:
                continue
            
            value = metrics[metric]
            values = sector_values[metric]
        ...
        if sector not in sector_composite:
            sector_composite[sector] = []
        sector_composite[sector].append((cik, symbol, composite_score))
    
    # 섹터별 종합 랭킹 생성
    for sector, companies in sector_composite.items():
        sorted_companies = sorted(companies, key=lambda x: x[2], reverse=True)
...
def emit_after_csv(args, companies, obs_rows):
    try:
        ...
        emit_efin_ttl(companies or [], obs_rows or [], args.emit_ttl, benchmarks, rankings)
